...
public class Inventory : MonoBehaviour
{
    ...
    public void AddItem(int itemNumber)
    {
        foreach (KeyValuePair<string, int> items in item_dic)
        {
            ...
            if (int.Parse(TextCount[slotNumber].text) < itemMaxCount && item_dic.Keys.ElementAt(index_dic) == ItemDB.instance.itemDB[itemNumber].itemName)
            {
            ...
            else if (int.Parse(TextCount[index_dic].text) == itemMaxCount && item_dic.Keys.ElementAt(slotNumber) == ItemDB.instance.itemDB[itemNumber].itemName)
            {
    ...
    {
        slotNumber += 1;
        Debug.Log(item_dic[ItemDB.instance.itemDB[itemNumber].itemName]);
    ...
    public void DrawItemCountUI(int itemNumber)
    {
        ...
        //Debug.Log(item_dic[ItemDB.instance.itemDB[itemNumber].itemName][i]);
        // 딕셔너리에서 Key로 인덱스 찾기
        index_dic = Array.IndexOf(item_dic.Keys.ToArray(), ItemDB.instance.itemDB[itemNumber].itemName);
        Debug.Log(index_dic);
